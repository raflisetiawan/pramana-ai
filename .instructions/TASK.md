...
- [x] **3.1.1** Download base model IndoBERT:

  ```bash
  # Di dalam service nlp-engine
  python -c "from transformers import AutoTokenizer, AutoModel; \
    AutoTokenizer.from_pretrained('indobenchmark/indobert-base-p1', cache_dir='./app/saved_models'); \
    AutoModel.from_pretrained('indobenchmark/indobert-base-p1', cache_dir='./app/saved_models')"
  ```

- [x] **3.1.2** Setup spaCy untuk Indonesian:

  ```bash
  python -m spacy download xx_ent_wiki_sm  # Multilingual model sebagai base
  ```

- [x] **3.1.3** Buat `services/nlp-engine/app/pipeline/ner.py`:
...
- [x] **3.2.1** Load ICD-10 dan ICD-9 master data ke memory saat startup
- [x] **3.2.2** Buat `services/nlp-engine/app/pipeline/icd_mapper.py`:
...
- [x] **3.2.3** Buat mapping manual untuk diagnosa Indonesia yang umum:
...
- [x] **3.3.1** Buat `services/nlp-engine/app/pipeline/confidence.py`:
...
- [x] **3.4.1** Buat FastAPI project di `services/nlp-engine/`
- [x] **3.4.2** `POST /nlp/extract-coding` — sesuai contract di `SPEC.md` bagian 5
  - Input: resume medis teks + context (tanggal, dll)
  - Output: kode ICD-10 utama + sekunder + prosedur + confidence + flag
- [x] **3.4.3** `POST /nlp/audit-consistency` — audit kode yang sudah ada:
  - Input: resume medis + kode yang diklaim RS
  - Output: apakah kode sesuai dengan resume? mismatch detail?
- [x] **3.4.4** `GET /nlp/health`
- [x] **3.4.5** Integrasi ke API Gateway: setelah klaim masuk, otomatis panggil NLP Engine

#### 3.5 Testing Phase 3

- [x] **3.5.1** Buat 10 resume medis sintetis (fiktif) sebagai test fixtures
- [x] **3.5.2** Unit test: ICD mapper dengan berbagai ejaan (typo, singkatan)
- [x] **3.5.3** Unit test: confidence scorer
- [x] **3.5.4** Integration test: resume medis → kode ICD lengkap
- [x] **3.5.5** Performance test: single extraction < 2 detik (CPU-only)
